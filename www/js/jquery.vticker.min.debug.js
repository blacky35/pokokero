...
bindMousePausing:function(a){
    ...
    setItemLayout:function(a,b,c){
      var f;
      a.css({overflow:"hidden",position:"relative"}).children("ul").css({position:"absolute",margin:0,padding:0}).children("li").css({margin:c.margin,
padding:c.padding});
      return isNaN(c.height)||0===c.height
                                 ?(a.children("ul").children("li").each(
                                    function(){
                                  ...
                                  ),f=c.margin+2*c.padding,a.height((b.itemHeight+f)*c.showItems+c.margin))
                                 :a.height(c.height)
    },
...
d(this).removeClass("paused");
    ...
    next:function(a){
      console.log("vTicker next");
      var b;
      b=c.getState("next",this);
      if(c.isAnimatingOrSingleItem(b)) {
        console.log("vTicker next return 1");
        return!1;
      }
      c.restartInterval.call(this);
      console.log("vTicker next return 2");
      return c.moveUp(b,a)
    },
    prev:function(a){
      var b;
